CreateTalents("npc_dota_hero_lina", "hero/hero_lina")
...
function modifier_imba_lina_light_strike_array_thinker_son:OnDestroy()
	if IsServer() then
		...
		for _, enemy in pairs(enemies) do
			enemy:AddNewModifier_RS(self:GetCaster(), self:GetAbility(), "modifier_imba_stunned", {duration = self:GetAbility():GetSpecialValueFor("stun_duration")})
			local damageTable = {
								victim = enemy,
								attacker = self:GetCaster(),
...
function modifier_imba_fiery_soul_passive:OnAbilityFullyCast(keys)
	...
	if not IsTriggerByAbility[keys.ability:GetAbilityName()] then
		...
		if not self:GetParent():HasModifier("modifier_imba_fiery_soul_active") or self:GetParent():TG_HasTalent("special_bonus_imba_lina_1") then
...
function modifier_imba_fiery_soul_stacks:DeclareFunctions() return {MODIFIER_PROPERTY_MOVESPEED_BONUS_CONSTANT, MODIFIER_PROPERTY_ATTACKSPEED_BONUS_CONSTANT} end
function modifier_imba_fiery_soul_stacks:GetModifierMoveSpeedBonus_Constant() return (self:GetStackCount() * self:GetAbility():GetSpecialValueFor("bonus_ms")) end
...
function imba_lina_laguna_blade:GetDamageType() 			return self:GetCaster():HasScepter() and 4 or 2 end
...
function imba_lina_laguna_blade:OnSpellStart()
	...
	if   target:TG_TriggerSpellAbsorb(self)  and not caster:Has_Aghanims_Shard()  then
		return
	end
