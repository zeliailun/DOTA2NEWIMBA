...
function imba_mirana_arrow:OnProjectileHit_ExtraData(target, location, keys)
	...
	target:AddNewModifier_RS(self:GetCaster(), self, "modifier_imba_stunned", {duration = stun_duration})
	EntIndexToHScript(keys.thinker):ForceKill(false)
	local damageTable = {
						victim = target,
...
function modifier_imba_leap:GetModifierAttackSpeedBonus_Constant() return (self:GetAbility():GetSpecialValueFor("leap_speedbonus_as") + self:GetCaster():TG_GetTalentValue("special_bonus_imba_mirana_2")) end
...
function imba_mirana_moonlight_shadow:GetCooldown()				return self:GetSpecialValueFor("cd") + self:GetCaster():TG_GetTalentValue("special_bonus_imba_mirana_1") end
...
function modifier_imba_moonlight_shadow_caster:GetAuraRadius() return 3000 end
