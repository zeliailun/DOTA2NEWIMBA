...
function modifier_imba_jinada_passive:OnAttackLanded(keys)
	...
	if self.crit[keys.record] then
		...
		PlayerResource:ModifyGold(keys.target:GetPlayerOwnerID(), (0 - (self:GetAbility():GetSpecialValueFor("gold_steal")+keys.damage*0.1)), false, DOTA_ModifyGold_Unspecified)
		PopupNumbers(keys.target, "gold", Vector(255, 200, 33), 1.0, (self:GetAbility():GetSpecialValueFor("gold_steal")+keys.damage*0.1), 1)
		PlayerResource:ModifyGold(self:GetParent():GetPlayerOwnerID(), (self:GetAbility():GetSpecialValueFor("gold_steal")+keys.damage*0.1), true, DOTA_ModifyGold_Unspecified)
		PopupNumbers(self:GetParent(), "gold", Vector(255, 200, 33), 1.0, (self:GetAbility():GetSpecialValueFor("gold_steal")+keys.damage*0.1), 0)
	end
