...
LinkLuaModifier("modifier_imba_thirst_invis_target", "linken/hero_bloodseeker.lua", LUA_MODIFIER_MOTION_NONE) 
...
function modifier_imba_thirst_invis:IsDebuff()			return false end
function modifier_imba_thirst_invis:IsHidden() 			return true end
...
function modifier_imba_thirst_invis:OnCreated()
	...
	if IsServer() then
		...
		self.parent, 
		self.ability, 
		"modifier_imba_thirst_invis_target",
		{
			duration = -1 ,
		}
		)
...
modifier_imba_thirst_invis_target = class({})
function modifier_imba_thirst_invis_target:IsDebuff()			return true end
function modifier_imba_thirst_invis_target:IsHidden() 			return false end
function modifier_imba_thirst_invis_target:IsPurgable() 		return false end
function modifier_imba_thirst_invis_target:IsPurgeException() 	return false end
function modifier_imba_thirst_invis_target:CheckState()
	return {[MODIFIER_STATE_INVISIBLE] = false } 
end
function modifier_imba_thirst_invis_target:GetPriority() return MODIFIER_PRIORITY_HIGH end
function modifier_imba_thirst_invis_target:GetModifierProvidesFOWVision() 
	return 1	 
end
function modifier_imba_thirst_invis_target:DeclareFunctions() 
    return 
    {
        MODIFIER_PROPERTY_PROVIDES_FOW_POSITION,
   	} 
end
function modifier_imba_thirst_invis_target:OnCreated()
	self.parent 	= 	self:GetParent()
	self.caster 	= 	self:GetCaster()
	self.ability 	= 	self:GetAbility()
	if IsServer() then
...
function modifier_imba_thirst_invis_target:OnIntervalThink()
	if not self.parent:HasModifier("modifier_imba_thirst_invis") then	
		self:Destroy()
	end
end
